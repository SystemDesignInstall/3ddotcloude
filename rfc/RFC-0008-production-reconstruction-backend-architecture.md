...
The LingBot family (Map/Vision/Video/VA/World/VLA) is **currently not eligible for production integration due to failed repository/source verification** (`github.com/LingBot/LingBot-Map` returns 404, verified 2026-08-12); it **remains a future research candidate, subject to re-evaluation through the same backend decision gate**. This is a statement about current evidence, not a permanent rejection: if an accessible, documented upstream with a verifiable license/API/output contract appears, the M2 gate is re-run.
