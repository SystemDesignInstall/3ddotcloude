...
- **Step 12 (CLI real providers + CLI E2E through the SOLE user entry point, PROVEN):**  the CLI
  composition root injects the REAL seam implementations — `L2NearestMatcher` +
  ...
  the `--input <file>`/`--config <json>` run path, and the exit code — `RunCommand` returns 0 ONLY
  ...
  **positive** — exit 0, manifest "succeeded" with BOTH stages succeeded, and the FULL v4 chain
  ...
  and the GTSAM trajectory optimization converged with the node pulled ≥ 5 cm off the drift;
  **negative** — a fail-closed config (missing `random_seed`, D6) yields exit code 1 with a
  ...
  AC-22 (PROVEN 2026-09-12: `AC22_DeterministicSecondRunThroughProductionSurface` twin — two fresh
