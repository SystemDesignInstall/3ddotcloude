...
// ===========================================================================
// AC-22 — DETERMINISTIC SECOND RUN THROUGH THE PRODUCTION SURFACE.
// ===========================================================================
//
// Two INDEPENDENT runs of the FULL corrected chain — each driven by ONE
...
// GtsamTrajectoryOptimizer + GtsamBundleAdjustmentOptimizer) — over
...
// is bit-deterministic — the measurement path (fixed-seed RANSAC over fixed
// pixels, plus the pure D6 unit->metric resolution against fixed trajectory
// nodes; the pose-graph edge is a VERBATIM copy of that measurement) — and
...
//   kAc22PoseTol      = 1e-9  — optimized/optimizer-fed pose arrays and BA
//                               rms/mean px telemetry (9 orders below any
//                               measurement significance);
//   kAc22GeometryTol  = 1e-6  — re-triangulated / BA-refined 3D world points
...
// documented reason to widen a kAc22*Tol bound — NOT a reason to weaken a
// semantic comparison below it.
//
// Runtime-only identity (fresh UUIDs — closure_id, graph_id, result_id,
...
class Ac22DeterminismTest : public RealGtsamBundleAdjustmentTest {
 protected:
  ...
  void ExpectCorrectionRevisionsEqual(const Ac22TwinEvidence& a,
    ...
    // options, pinned seed) — deliberately NOT the project-specific pipeline
    // hash — so it agrees across the twins and pins the D6 seed.
...
TEST_F(Ac22DeterminismTest,
  ...
  // ── Input identity: the ONLY config divergence is the owning project.
  ...
  // ── Level A semantics: measurement -> graph -> optimization -> revisions.
  ...
  // ── Level B provenance + lineage: identical chain topology and roots.
  ...
  // Provenance: the chain's deterministic root — the committed reconstruction
  // payload, whose CAS content hash is bit-identical across the twins — is
  ...
  // differ between the twins by contract; what must agree is the topology.
  {
    ...
    // persisted optimization-result payload } (reconstruction_feedback.h §8.1).
    ...
    // reconstruction id (triangulation.h §4.11): same topology in both twins,
  ...
  // ── Latest-succeeded view agrees (only the fresh v4 id differs).
  {
    const auto scene_a =
  ...
  // ── N5 replay with the REAL seams, inside run A's project: an identical
  // invocation (same payload + same config) re-executes the kNever stage and
  // FAILS CLOSED at the duplicate committed identity — never a cache hit, never
